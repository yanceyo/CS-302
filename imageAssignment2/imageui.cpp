	...
	if(image)
	{
		...
		if(!value.isEmpty())
		{
			if((value.toInt()>-1) && (value.toInt()<=255))
			{
				...
				int labelNumber = computeComponents(&tempImage, &outImage, 1);
	...
	if(image)
	{
		...
		if(!value.isEmpty())
		{
			if((value.toInt()>-1) && (value.toInt()<=255))
			{
				...
				// set computeComponents() output
				int labelNumber = computeComponents(&tempImage, &outImage, 2);
	...
	if(image)
	{
		...
		if(!value.isEmpty())
		{
			if((value.toInt()>-1) && (value.toInt()<=255))
			{
				...
				// set computeComponents() output
				int labelNumber = computeComponents(&tempImage, &outImage, 3);
...
int imageUI::computeComponents(Image * inputImage, Image * outputImage, int flag) //
{
...
void imageUI::findComponentDFS(Image * inputImage, Image * outputImage, int i, int j, int label) // Caitlin
{
	int N,M,Q;
	inputImage->getImageInfo(N,M,Q);
